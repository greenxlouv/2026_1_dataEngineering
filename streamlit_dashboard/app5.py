...
if not TOP_MENUS_READY:
    ...
    for i, menu in enumerate(top_menus[:5], 1):
        ...
        menus_html += (
            ...
            f"<span style='color:#bbb; font-size:0.85rem; width:20px;'>{i}</span>"
            f"<span style='flex:1; margin-left:0.5rem; font-weight:500;'>{menu_name}</span>"
            "</div>"
        )
else:
...
# 방문 목적 분포 HTML
if purpose_dist:
    max_cnt = purpose_dist.most_common(1)[0][1]
    dist_html = ""
    for code, cnt in purpose_dist.most_common(5):
        ...
        dist_html += (
            ...
            f"<span>{label}</span><span style='color:#888;'>{cnt:,}</span>"
            "</div>"
            f"<div style='background:#d0d0d0; border-radius:4px; height:8px; margin-top:0.3rem;'>"
            f"<div style='background:#66bb6a; border-radius:4px; height:8px; width:{pct:.1f}%;'></div>"
            "</div>"
            "</div>"
        )
else:
...
<div style='display:flex; gap:1rem; margin-bottom:1rem;'>
    <div style='flex:1; background:white; border-radius:12px; padding:1.4rem 1.6rem; box-shadow:0 1px 4px rgba(0,0,0,0.06);'>
        <div style='font-size:22px; font-weight:600; margin-bottom:1rem;'>실제 대표메뉴 (전체 리뷰 기반)</div>
        {menus_html}
    </div>
    <div style='flex:1; background:white; border-radius:12px; padding:1.4rem 1.6rem; box-shadow:0 1px 4px rgba(0,0,0,0.06);'>
        <div style='font-size:22px; font-weight:600; margin-bottom:1rem;'>방문 목적 분포</div>
        {dist_html}
    </div>
</div>
""", unsafe_allow_html=True)
...
# =========================================================
db = get_db()
tags_doc = db.sentiment_tags.find_one({"restaurant_id": rid})

# 2. 데이터 유무에 따른 안전한 태그 매핑
if tags_doc and "tags" in tags_doc:
    db_tags = tags_doc["tags"]
    review_tags = {
        "positive": db_tags.get("positive", []),
        "warning": db_tags.get("warning", []),
        "revisit": db_tags.get("revisit", [])  # menu 대신 revisit 사용
    }
else:
    # DB에 아직 분석 데이터가 없는 경우 처리
    review_tags = {
        "positive": [],
        "warning": [],
        "revisit": []
...
# 3. 태그 렌더링 함수 (빈 배열일 경우 예외 처리)
def make_tags(tags, cls):
    if not tags:
        return "<span style='color:#bbb; font-size:0.8rem; margin-left:0.2rem;'>데이터 없음</span>"
...
<div style='background:white; border-radius:12px; padding:1.4rem 1.6rem;
    ...
    <div style='font-size:22px; font-weight:600; margin-bottom:1rem;'>AI 핵심 감성 태그</div>
    <div style='display:flex; gap:2rem;'>
        ...
        <div>
            <div style='font-size:0.85rem; font-weight:600; margin-bottom:0.5rem;'>⚠️ 주의</div>
            {make_tags(review_tags["warning"], "tag-warn")}
        </div>
        <div>
            <div style='font-size:0.85rem; font-weight:600; margin-bottom:0.5rem;'>✨ 재방문</div>
            {make_tags(review_tags["revisit"], "tag-menu")}
        </div>
    </div>
</div>
""", unsafe_allow_html=True)
...
if not TOP_MENUS_BY_PURPOSE_READY:
    ...
    <div style='background:white; border-radius:12px; padding:1.4rem 1.6rem; box-shadow:0 1px 4px rgba(0,0,0,0.06);'>
        ...
        <div style='background:#fff8e1; border:1px dashed #ffd54f; border-radius:8px; padding:0.8rem; color:#f57f17; text-align:center; font-size:0.85rem;'>
            top_menus_by_purpose DB 적재 완료 후 True로 변경
        </div>
    </div>
    """, unsafe_allow_html=True)
elif by_purpose:
        ...
        for code, menus in row:
            ...
            for i, menu in enumerate(menus[:5], 1):
                ...
                menu_rows_html += (
                    ...
                    f"<span style='color:#555; font-size:0.82rem; width:18px;'>{i}</span>"
                    f"<span style='margin-left:0.5rem; font-size:0.9rem; color:#1a1a1a;'>{menu_name}</span>"
                    "</div>"
                )

            cols_html += (
                f"<div style='flex:1; background:#ede9df; border-radius:12px; padding:1.2rem 1.4rem; height:{box_height}px; box-sizing:border-box;'>"
                f"<div style='display:inline-block; background:#e8f5f0; color:#2e8b6e; font-size:0.8rem; font-weight:600; padding:0.25rem 0.7rem; border-radius:20px; margin-bottom:0.8rem;'>{label}</div>"
                + menu_rows_html +
                "</div>"
            )

        for _ in range(3 - len(row)):
            cols_html += "<div style='flex:1; visibility:hidden;'></div>"

        all_rows_html += "<div style='display:flex; gap:1rem; margin-bottom:1rem; width:100%;'>" + cols_html + "</div>"

    purpose_html = (
        "<div style='background:white; border-radius:12px; padding:1.4rem 1.6rem; box-shadow:0 1px 4px rgba(0,0,0,0.06);'>"
        "<div style='font-size:22px; font-weight:600; margin-bottom:1rem;'>방문 목적별 인기 메뉴</div>"
        + all_rows_html +
        "</div>"
    )
