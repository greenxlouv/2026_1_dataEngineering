...
실행:
  python3 menu_extract_final.py
  python3 menu_extract_final.py --min-reviews 100 --workers 8 --sample 20
"""
...
def process_restaurant(args):
    ...
    try:
        ...
        # [수정] MongoDB 저장
        if not sample_mode or test_mode:
            col_name = "Restaurants_test" if test_mode else "Restaurants"
...
def main():
    ...
    print(f"저장 컬렉션: {'Restaurants_test' if test_mode else 'Restaurants'}")
    print("=" * 60)
    ...
    valid_ids = [r["_id"] for r in db.Reviews.aggregate(pipeline)]
    print(f"  → 유효 리뷰 {args.min_reviews}개+ 식당: {len(valid_ids)}개")

    # 메뉴 있는 식당 / 없는 식당 분리
    ids_with_menus = set(db.Menus.distinct("restaurant_id", {"restaurant_id": {"$in": valid_ids}}))
